...
QUEUES_TO_CREATE = [DEFAULT_QUEUE_NAME] + %w(default_tracking job_tracking tracking small medium large one two)
...
RSpec.configure do |config|
  ...
  config.before(:each, :sqs) do
    $fake_sqs.reset
    QUEUES_TO_CREATE.each{ |q_name| $sqs.create_queue(queue_name: q_name) }
  end
